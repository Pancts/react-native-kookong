        ...
        KookongSDK.getBrandListFromNet(deviceType, new IRequestResult<BrandList>()
        {
            ...
            public void onSuccess(String msg, BrandList result) {
                ...
                for (int i = 0; i < stbs.size(); i++) {
                    ...
                    brandItem.put("cname", stbs.get(i).cname);
                    brandItem.put("ename", stbs.get(i).ename);
                    // brandItem.put("pinyin", stbs.get(i).pinyin);
                    brandItem.put("initial", stbs.get(i).initial);
        ...
        KookongSDK.getIRDataById(rid, deviceType, new IRequestResult<IrDataList>()
        {
            ...
            public void onSuccess(String msg, IrDataList result) {
                ...
                if (mIrData.type == 2) {
                    ...
                    Integer[] wrappedInts = toObject(patternsInArray);
                    successCallback.invoke(ArrayUtil.toWritableArray((Object[]) wrappedInts));
                }else{
                    List<IrData.IrKey> mIrKeys = mIrData.keys;
                    final IrData.IrKey irKey = mIrKeys.get(0);
                    Logger.d("remote (" + rid + ") key (" + irKey.fkey + ") is " + irKey.pulse);
                    String[] array = irKey.pulse.split(",");
                    final Integer[] resultArr = new Integer[array.length];
                    for (int i = 0; i < array.length; i++) {
                        resultArr[i] = new Integer(array[i]);
                    }
                    successCallback.invoke(ArrayUtil.toWritableArray((Object[]) resultArr));
//                    JSONObject patternsInArray = new JSONObject(irKey.pulse);
//                    successCallback.invoke(ArrayUtil.toWritableArray((Object[]) ArrayUtil.toArray(patternsInArray)));
                }
            }
            ...
            public void onSuccess(String msg, IrDataList result) {
                ...
                if (mIrData.type == 2) {
                   ...
                   successCallback.invoke(ArrayUtil.toWritableArray((Object[]) wrappedInts));
                    ...
                    successCallback.invoke(ArrayUtil.toWritableArray((Object[]) resultArr));
//                    successCallback.invoke(ArrayUtil.toWritableArray(patternsInArray));
